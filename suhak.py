...
# ---------------------------------------------------------
st.set_page_config(page_title="공통수학 1 튜터 봇", page_icon="📚", layout="centered")
st.title("📚 공통수학 1 튜터 봇 (PDF 지원)")
st.caption("경우의 수, 순열과 조합 문제의 PDF를 업로드하고 풀이 방향을 질문해 보세요!")

if "messages" not in st.session_state:
    system_prompt = """
    당신은 고등학교 공통수학 1의 '경우의 수', '순열', '조합' 단원에 특화된 친절하고 논리적인 수학 튜터입니다.
    학생이 문제를 제시하거나 업로드한 PDF 내용을 바탕으로 질문하면 정답이나 최종 계산식을 바로 알려주지 마세요.
    대신 다음 가이드라인을 따르세요:
    1. 문제의 핵심을 파악하여 어떤 개념(합의 법칙, 곱의 법칙, 순열, 조합 등)을 적용해야 하는지 설명합니다.
    2. 문제를 해결하기 위해 어떤 단계로 나누어 생각해야 하는지 풀이의 '접근 방향'과 '논리적 흐름'을 제시합니다.
    3. 조건이 복잡한 경우, 케이스를 어떻게 분류하면 좋을지 힌트를 제공합니다.
    4. 학생이 스스로 식을 세우고 계산할 수 있도록 유도하는 질문으로 마무리합니다.
    """
